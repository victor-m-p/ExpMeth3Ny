...
```{r}
LU_train_test <- read_csv("/Users/pernillebrams/Desktop/EM3/ExpMeth3Ny/methods3_A1-master/LU_train.csv")
LU_train_test$VISIT <- data.frame(str_extract(LU_train_test$VISIT, "\\d"))
...
```{r}
#str_replace_all(, "\\s+", " ") #how to use str_replace_all(text, "\\s+", " ")


# USE STR REPLACE ALL - tidyvserse
```
...
rel_demo_train <- select(demo_train, SUBJ, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS,  MullenRaw, ExpressiveLangRaw, Socialization)

rel_LU_train <- select(LU_train, SUBJ, VISIT, MOT_MLU, CHI_MLU)

rel_token_train <- select(token_train, SUBJ, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
combined_data <- merge(rel_demo_train, rel_LU_train)

#new dataframe with all dataframes merged
all_data <- merge(combined_data, rel_token_train)
...
```{r}
# making dataset with only visit 1, SUBJ (before child-id) and the 4 relevant clinical variables to be merged with the old dataset
only_visit1 <- filter(all_data, all_data$VISIT == 1)
